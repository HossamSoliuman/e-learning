...
class CurrencyController extends Controller {
    ...
    public function destroy($id) {
        ...
        $basicPaymentKeys = ['stripe_currency_id', 'paypal_currency_id', 'bank_currency_id'];
        if ($basicPayments = BasicPayment::whereIn('key', $basicPaymentKeys)->get()) {
            foreach ($basicPayments as $basicPayment) {
                if ($basicPayment->value == $id) {
                    return redirect()->route('admin.currency.index')->with([
                        'alert-type' => 'error',
                        'messege'    => __('Currency can not be deleted'),
                    ]);
                }
            }
        }
        $paymentGetwayKeys = ['flutterwave_currency_id', 'razorpay_currency_id', 'paystack_currency_id', 'mollie_currency_id', 'instamojo_currency_id'];
        if ($paymentGetways = PaymentGateway::whereIn('key', $paymentGetwayKeys)->get()) {
            foreach ($paymentGetways as $paymentGetway) {
                if ($paymentGetway->value == $id) {
                    return redirect()->route('admin.currency.index')->with([
                        'alert-type' => 'error',
                        'messege'    => __('Currency can not be deleted'),
                    ]);
